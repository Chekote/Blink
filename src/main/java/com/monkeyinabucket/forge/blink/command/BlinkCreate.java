...
public class BlinkCreate extends BaseCommand implements ICommand {
  ...
  public void processCommand(ICommandSender sender, String[] command) {
    ...
    int distance = 64;
    MovingObjectPosition targetPos = player.rayTrace(distance, 1.0F);

    World world = player.getEntityWorld();

    int center = Blink.halfRuneSize;
    int cornerX = targetPos.blockX - Blink.halfRuneSize;
    int cornerZ = targetPos.blockZ - Blink.halfRuneSize;
    int y = targetPos.blockY;
